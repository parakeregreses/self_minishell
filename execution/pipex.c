...
/*   Updated: 2025/08/23 16:01:36 by jlaine-b         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pipex(t_exec *infos, int n, char ***envp, int *status)
{
	...
	while (i < n)
	{
		if (i % 2 == 0)
		{
			...
			execution(infos[i], pipe2, pipe1, i, envp, saved_stdin, saved_stdout, status, n);
		}
		if (i % 2 != 0)
		{
			if (pipe(pipe2) == -1)
			...
			execution(infos[i], pipe1, pipe2, i, envp, saved_stdin, saved_stdout, status, n);
		}
		i++;
	}
